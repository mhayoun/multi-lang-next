...
import {
    ...
    GripVertical, Copy, Check, Video, ExternalLink, Sparkles, Loader2
} from 'lucide-react';
...
const SubMenuEditor = ({
    ...
    const handleAIGenerate = async () => {
        ...
        try {
            const API_KEY = "AIzaSyBsbMetRcD3DKrDMwggT0NWFHaNgDWeJpI";

            // Using the most widely supported specific version for v1beta
            const URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_KEY}`;

            const systemPrompt = "Produce html code format without html,head,body tags, only format by keeping the data provided: ";
            ...
            const response = await fetch(URL, {
                ...
                body: JSON.stringify({
                    contents: [{
                        parts: [{text: systemPrompt + currentText}]
                    }]
                })
            });
            ...
            if (!response.ok) {
                // If the server is just busy/overloaded
                if (response.status === 503) {
                    throw new Error("Server is temporarily busy. Please click again in a few seconds.");
                }
            ...
            if (rawHtml) {
                // Clean markdown and illegal root tags
                    ...
                    <div className="bg-white w-full h-full rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                        <div className="p-4 border-b flex justify-between items-center bg-slate-50">
                            <div className="flex items-center gap-3">
                                ...
                                {/* --- NEW GENERATE AI BUTTON --- */}
                                <button
                                    onClick={handleAIGenerate}
                                    disabled={isGenerating}
                                    className={`flex items-center gap-2 text-[10px] px-3 py-1.5 rounded-md font-bold transition border shadow-sm ${
                                        isGenerating
                                            ? 'bg-slate-100 text-slate-400 cursor-not-allowed'
                                            : 'bg-gradient-to-r from-purple-500 to-blue-500 text-white hover:from-purple-600 hover:to-blue-600 border-transparent'
                                    }`}
                                >
                                    {isGenerating ? <Loader2 size={12} className="animate-spin"/> :
                                        <Sparkles size={12}/>}
                                    {isHe ? (isGenerating ? 'מעבד...' : 'שיפור AI') : (isGenerating ? 'Processing...' : 'Generate AI')}
                                </button>

                            </div>
                            <button onClick={() => logic.setIsModalOpen(false)}
                                    className="p-2 hover:bg-red-50 text-slate-400 hover:text-red-500 rounded-full transition">
                                <X size={24}/></button>
                        </div>
